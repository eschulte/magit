...
(defun magit-git-full (&rest args)
  "Execute Git with ARGS, returning its full output as a string."
  (with-temp-buffer
    (apply 'process-file magit-git-executable nil (list t nil) nil
           (append magit-git-standard-options args))
    (buffer-string)))
...
(defun magit-format-patch (&optional as-message)
  ...
  (let* ((since (magit-read-rev-with-default "since: "))
         (output
          (apply #'magit-git-full "format-patch"
                 (append
                  (mapcar
                   (lambda (opt)
                     (if (string-match "--output-directory=\\(.*\\)$" opt)
